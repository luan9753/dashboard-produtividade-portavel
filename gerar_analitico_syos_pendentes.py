"""Analítico completo: TODOS os Syos pendentes (entregues no stage, sem cadastro na orders)."""
...
def main() -> None:
    ...
    base = df[(df["status"] == "pendente") & (df["equipamento"] == "Syos")].copy()
    ...
    out = pd.DataFrame(
        {
            ...
            "Qtd Syos pendentes no pedido": qtd_por_pedido.values,
        }
    )
    ...
    xlsx = OUT / "syos_pendentes_analitico.xlsx"
    csv = OUT / "syos_pendentes_analitico.csv"
    ...
    antes.to_csv(
        OUT / "syos_pendentes_antes_27-05-2026.csv",
        index=False,
        sep=";",
        encoding="utf-8-sig",
    )
    apos.to_csv(
        OUT / "syos_pendentes_apos_27-05-2026.csv",
        index=False,
        sep=";",
        encoding="utf-8-sig",
    )
